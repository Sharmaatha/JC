...
def enrich_social_links(limit: Optional[int] = None, product_ids: Optional[List[int]] = None, is_automatic: bool = False, scrape_date: Optional[str] = None):
    from datetime import datetime
    ...
    try:
        ...
        with Database() as db:
            ...
            # For automatic runs, only enrich products from the specified scrape date
            ...
            for product in products:
                try:
                    ...
                    print(f"❌ Error enriching product {product.id}: {e}")
                    logger.error(f"Error enriching product {product.id}: {e}")
                    # Continue to next product instead of crashing
                    continue

    except Exception as e:
        print(f"❌ Critical error in enrich_social_links: {e}")
        logger.error(f"Critical error in enrich_social_links: {e}")
        raise